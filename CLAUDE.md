...
- **Vue opération = vues EN PLACE** (le système tiroir/drawer a été retiré) : bandeau unique `opsUnifiedBarHtml` (nav Vue d'ensemble / Informations / Bilan / Financements / Suivi + scope « Total / tranche »), strip KPI permanent, dashboard à briques (`renderOpHomeDashboard`), vue financements générée (`renderOpFinancementsDrawer`, couleurs par tranche). **Scope consultation** : `TR_SCOPE` (null = Total) + `setTrScope` - une tranche sélectionnée fait afficher son détail à la place d'Informations (`tr`), et filtre Bilan et Financements sur sa part ; en édition le sélecteur garde le comportement historique (Détail/Financements de saisie). Routage : `switchOpsTab` + `OPS_TAB` ('home','syn','dos','bilan','suivi','tr','fin','finop') ; `openOpsDomain` est un simple routeur ; Échap revient à 'home'.
